<template>
  <section class="container mx-auto w-full">
    <form ref="dropdownSearchBox" class=" md:mx-auto mx-4 bg-white rounded shadow-md outline-none mt-4 flex items-center" style="height:4.5rem;" @click.prevent>
        <input  @keyup.enter="search()" v-model="searchQuery"   class="md:w-full py-4 px-6 font-normal text-xl outline-none" type="search" placeholder="Search Crypto Currency..."/>
        <button @click="search()" ref="searchButton" class="bg-brand-blue outline-none py-3 px-4 rounded text-white mr-8">
